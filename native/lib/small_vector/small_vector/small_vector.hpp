...
#    if defined(GCH_HAS_CPP20_CONSTEXPR) && defined(GCH_LIB_IS_CONSTANT_EVALUATED) \
        && defined(GCH_LIB_CONSTEXPR_MEMORY)
#        define GCH_SMALL_VECTOR_CONSTEXPR constexpr
#        ifndef GCH_HAS_CONSTEXPR_SMALL_VECTOR
#            define GCH_HAS_CONSTEXPR_SMALL_VECTOR
#        endif
#    else
#        define GCH_SMALL_VECTOR_CONSTEXPR
#    endif
#endif
...
#    endif
        ...
        template <typename T, typename X,
                  typename A
                  = typename std::conditional<requires { typename X::allocator_type; },
        ...
        concept Erasable
            = std::same_as<typename X::value_type, T>
              && ((requires { typename X::allocator_type; }  // if X is allocator aware
                   && std::same_as<typename X::allocator_type,
                                   typename std::allocator_traits<A>::template rebind_alloc<T>>
                   && (requires(A m, T* p) { m.destroy(p); } || std::is_destructible<T>::value))
                  || (!requires { typename X::allocator_type; } && std::is_destructible<T>::value));
        ...
        concept NullablePointer
            = EqualityComparable<T> && DefaultConstructible<T> && CopyConstructible<T>
              && CopyAssignable<T> && Destructible<T> && ConstructibleFrom<T, std::nullptr_t>
              && ConvertibleTo<std::nullptr_t, T> && requires(T p, T q, std::nullptr_t np) {
                                                         { T(np) } -> std::same_as<T>;
                                                         { p = np } -> std::same_as<T&>;
                                                         {
                                                             p != q
        ...
        concept AllocatorFor = NoThrowCopyConstructible<A> && requires(A a, typename std::allocator_traits<A>::template rebind_alloc<U> b, U xp, typename std::allocator_traits<A>::pointer p, typename std::allocator_traits<A>::const_pointer cp, typename std::allocator_traits<A>::void_pointer vp, typename std::allocator_traits<A>::const_void_pointer cvp, typename std::allocator_traits<A>::value_type& r, typename std::allocator_traits<A>::size_type n) {
                                                                  /** Inner types **/
                                                                  // A::pointer
                                                                  ...
                                                                  -> std::same_as<const typename A::
                                                                                      value_type&>;

                                                                  // Language in the standard
                                                                  // implies that `decltype (p)`
                                                                  // must either be a raw pointer or
                                                                  // implement `operator->`. There
                                                                  // is no mention of
                                                                  // `std::to_address` or
    ...
    template <typename T, unsigned InlineCapacity = default_buffer_size<std::allocator<T>>::value,
    ...
    struct default_buffer_size {
      private:
        ...
        template <typename U> struct is_complete<U, decltype(static_cast<void>(sizeof(U)))>
            : std::true_type {};
      ...
      public:
        ...
        static constexpr unsigned ideal_total = GCH_SMALL_VECTOR_DEFAULT_SIZE;
    ...
    template <typename Allocator> GCH_INLINE_VARIABLE constexpr unsigned default_buffer_size_v
        = default_buffer_size<Allocator>::value;

#endif

    template <typename Pointer, typename DifferenceType> class small_vector_iterator {
      public:
        ...
        small_vector_iterator(void) = default;
#else
        constexpr small_vector_iterator(void) noexcept : m_ptr() {}
#endif

        constexpr explicit small_vector_iterator(const Pointer& p) noexcept : m_ptr(p) {}

        template <typename U, typename D,
                  typename std::enable_if<std::is_convertible<U, Pointer>::value>::type* = nullptr>
        constexpr GCH_IMPLICIT_CONVERSION small_vector_iterator(
            const small_vector_iterator<U, D>& other) noexcept
            : m_ptr(other.base()) {}

        GCH_CPP14_CONSTEXPR
        small_vector_iterator& operator++(void) noexcept {
            ++m_ptr;
            return *this;
        }

        GCH_CPP14_CONSTEXPR
        small_vector_iterator operator++(int) noexcept { return small_vector_iterator(m_ptr++); }
        ...
        GCH_CPP14_CONSTEXPR
        small_vector_iterator& operator+=(difference_type n) noexcept {
            m_ptr += n;
            return *this;
        }
        ...
        constexpr pointer operator->(void) const noexcept { return get_pointer(m_ptr); }
        ...
        constexpr const Pointer& base(void) const noexcept { return m_ptr; }
    ...
    constexpr bool operator==(const small_vector_iterator<PointerLHS, DifferenceTypeLHS>& lhs,
    ...
    template <typename Pointer, typename DifferenceType> constexpr bool operator==(
        const small_vector_iterator<Pointer, DifferenceType>& lhs,
        const small_vector_iterator<Pointer, DifferenceType>& rhs) noexcept(noexcept(lhs.base()
                                                                                     == rhs.base()))
        requires requires {
                     { lhs.base() == rhs.base() } -> std::convertible_to<bool>;
                 }
    ...
    constexpr auto operator<=>(const small_vector_iterator<PointerLHS, DifferenceTypeLHS>& lhs,
                               const small_vector_iterator<PointerRHS, DifferenceTypeRHS>&
                                   rhs) noexcept(noexcept(lhs.base() <=> rhs.base())) {
        return lhs.base() <=> rhs.base();
    }
    ...
    constexpr auto operator<=>(const small_vector_iterator<Pointer, DifferenceType>& lhs,
    ...
    constexpr auto operator<=>(
        ...
        const small_vector_iterator<PointerRHS, DifferenceTypeRHS>&
            rhs) noexcept(noexcept(lhs.base() < rhs.base()) && noexcept(rhs.base() < lhs.base())) {
    ...
    template <typename Pointer, typename DifferenceType> constexpr auto operator<=>(
        const small_vector_iterator<Pointer, DifferenceType>& lhs,
        const small_vector_iterator<Pointer, DifferenceType>&
            rhs) noexcept(noexcept(lhs.base() < rhs.base()) && noexcept(rhs.base() < lhs.base())) {
        using ordering = std::weak_ordering;
    ...
    template <typename Pointer, typename DifferenceType>
    constexpr bool operator==(const small_vector_iterator<Pointer, DifferenceType>& lhs,
    ...
    template <typename Pointer, typename DifferenceType> constexpr DifferenceType operator-(
        const small_vector_iterator<Pointer, DifferenceType>& lhs,
        const small_vector_iterator<Pointer, DifferenceType>& rhs) noexcept {
        return static_cast<DifferenceType>(lhs.base() - rhs.base());
    }
    ...
    constexpr small_vector_iterator<Pointer, DifferenceType> operator+(
        DifferenceType n, const small_vector_iterator<Pointer, DifferenceType>& it) noexcept {
        return it + n;
    }
    ...
    namespace detail {
        ...
        namespace small_vector_adl {
            ...
            template <typename T, typename Enable = void> struct is_nothrow_swappable
                : std::false_type {};
        ...
        template <typename T, unsigned InlineCapacity> class inline_storage {
          public:
            using value_ty = T;
            ...
            GCH_NODISCARD GCH_CPP14_CONSTEXPR value_ty* get_inline_ptr(void) noexcept {
                return static_cast<value_ty*>(static_cast<void*>(std::addressof(*m_data)));
            }

            GCH_NODISCARD constexpr const value_ty* get_inline_ptr(void) const noexcept {
                return static_cast<const value_ty*>(
                    static_cast<const void*>(std::addressof(*m_data)));
            }

            static constexpr std::size_t element_size(void) noexcept { return sizeof(value_ty); }

            static constexpr std::size_t alignment(void) noexcept { return alignof(value_ty); }

            static constexpr unsigned num_elements(void) noexcept { return InlineCapacity; }

            static constexpr std::size_t num_bytes(void) noexcept {
        ...
        template <typename T> class GCH_EMPTY_BASE inline_storage<T, 0> {
          public:
            using value_ty = T;
            ...
            static constexpr unsigned num_elements(void) noexcept { return 0; }

            static constexpr std::size_t num_bytes(void) noexcept { return 0; }
        };
        ...
        template <typename Allocator> class GCH_EMPTY_BASE allocator_inliner<Allocator, true>
            : private Allocator {
            ...
            GCH_CPP20_CONSTEXPR void maybe_assign(const allocator_inliner&) noexcept {}

            template <bool IsNoOp = copy_assign_is_noop,
                      typename std::enable_if<!IsNoOp, bool>::type = false>
            GCH_CPP20_CONSTEXPR void maybe_assign(const allocator_inliner& other) noexcept(
            ...
            template <bool IsNoOp = move_assign_is_noop,
                      typename std::enable_if<IsNoOp, bool>::type = true>
            GCH_CPP20_CONSTEXPR void maybe_assign(allocator_inliner&&) noexcept {}
          ...
          public:
            ...
            GCH_CPP20_CONSTEXPR
            allocator_inliner& operator=(const allocator_inliner& other) noexcept(
            ...
            GCH_CPP20_CONSTEXPR
            allocator_inliner& operator=(allocator_inliner&& other) noexcept(
                noexcept(std::declval<allocator_inliner&>().maybe_assign(std::move(other)))) {
                assert(&other != this
            ...
            GCH_CPP14_CONSTEXPR
            Allocator& allocator_ref(void) noexcept { return *this; }

            constexpr const Allocator& allocator_ref(void) const noexcept { return *this; }

            template <bool IsNoOp = swap_is_noop,
                      typename std::enable_if<IsNoOp, bool>::type = true>
            GCH_CPP20_CONSTEXPR void swap(allocator_inliner&) {}

            template <bool IsNoOp = swap_is_noop,
                      typename std::enable_if<!IsNoOp, bool>::type = false>
            GCH_CPP20_CONSTEXPR void swap(allocator_inliner& other) {
                using std::swap;
                swap(static_cast<Allocator&>(*this), static_cast<Allocator&>(other));
            }
        };

        template <typename Allocator> class allocator_inliner<Allocator, false> {
            using alloc_traits = std::allocator_traits<Allocator>;
          ...
          public:
            ...
            GCH_CPP20_CONSTEXPR explicit allocator_inliner(const Allocator& alloc) noexcept
                : m_alloc(alloc) {}
            ...
            GCH_CPP14_CONSTEXPR
            Allocator& allocator_ref(void) noexcept { return m_alloc; }

            constexpr const Allocator& allocator_ref(void) const noexcept { return m_alloc; }
        ...
        template <typename Allocator> class GCH_EMPTY_BASE allocator_interface
          ...
          protected:
            ...
            // Select the fastest types larger than the user-facing types. These are only intended
            // for internal computations, and should not have any memory footprint visible to
            // consumers.
            using size_ty = typename std::conditional<
                (sizeof(size_type) <= sizeof(std::uint8_t)), std::uint_fast8_t,
                typename std::conditional<
                    (sizeof(size_type) <= sizeof(std::uint16_t)), std::uint_fast16_t,
                    typename std::conditional<
                        (sizeof(size_type) <= sizeof(std::uint32_t)), std::uint_fast32_t,
                        typename std::conditional<(sizeof(size_type) <= sizeof(std::uint64_t)),
                                                  std::uint_fast64_t, size_type>::type>::type>::
                    type>::type;

            using diff_ty = typename std::conditional<
                (sizeof(difference_type) <= sizeof(std::int8_t)), std::int_fast8_t,
                typename std::conditional<
                    (sizeof(difference_type) <= sizeof(std::int16_t)), std::int_fast16_t,
                    typename std::conditional<
                        (sizeof(difference_type) <= sizeof(std::int32_t)), std::int_fast32_t,
                        typename std::conditional<(sizeof(difference_type) <= sizeof(std::int64_t)),
                                                  std::int_fast64_t,
                                                  difference_type>::type>::type>::type>::type;
          ...
          private:
            ...
            template <typename V, typename Enable = void> struct is_trivially_destructible
                : std::false_type {};

            template <typename V>
            struct is_trivially_destructible<V,
            ...
            template <typename V, typename... Args> struct is_trivially_constructible_impl<
                typename std::enable_if<is_complete<V>::value>::type, V, Args...>
                : std::is_trivially_constructible<V, Args...> {};

            template <typename V, typename... Args> struct is_trivially_constructible
                : is_trivially_constructible_impl<void, V, Args...> {};

            template <typename T, typename Enable = void> struct underlying_if_enum {
                using type = T;
            };
            ...
            template <typename T> using underlying_if_enum_t = typename underlying_if_enum<T>::type;

            template <typename, typename = void> struct has_ptr_traits_to_address
                : std::false_type {};

            template <typename P> struct has_ptr_traits_to_address<
                P, void_t<decltype(std::pointer_traits<P>::to_address(std::declval<P>()))>>
                : std::true_type {};
            ...
            struct has_alloc_construct_check<void_t<decltype(std::declval<A&>().construct(
                                                 std::declval<V*>(), std::declval<Args>()...))>,
                                             A, V, Args...> : std::true_type {};

            template <typename Void, typename A, typename V, typename... Args>
            struct has_alloc_construct_impl : std::false_type {};

            template <typename A, typename V, typename... Args>
            struct has_alloc_construct_impl<typename std::enable_if<is_complete<V>::value>::type, A,
                                            V, Args...>
                : has_alloc_construct_check<void, A, V, Args...> {};

            template <typename A, typename V, typename... Args> struct has_alloc_construct
                : has_alloc_construct_impl<void, A, V, Args...> {};

            template <typename A, typename V, typename... Args> struct must_use_alloc_construct
                : bool_constant<!std::is_same<A, std::allocator<V>>::value
                                && has_alloc_construct<A, V, Args...>::value> {};

            template <typename Void, typename A, typename V> struct has_alloc_destroy_impl
                : std::false_type {};

            template <typename A, typename V> struct has_alloc_destroy_impl<
                void_t<decltype(std::declval<A&>().destroy(std::declval<V*>()))>, A, V>
                : std::true_type {};

            template <typename A, typename V, typename Enable = void> struct has_alloc_destroy
                : std::false_type {};
            ...
            template <typename A, typename V> struct must_use_alloc_destroy
                : bool_constant<!std::is_same<A, std::allocator<V>>::value
          ...
          public:
            ...
            template <typename, typename, typename = void> struct is_memcpyable_integral
                : std::false_type {};
            ...
            template <typename From, typename To> struct is_convertible_pointer
                : bool_constant<std::is_pointer<From>::value && std::is_pointer<To>::value
                                && std::is_convertible<From, To>::value> {};
            ...
            struct is_memcpyable<QualifiedFrom, QualifiedTo,
                ...
                static constexpr bool value
                    = std::is_trivially_assignable<QualifiedTo&, QualifiedFrom>::value
            ...
            struct is_uninitialized_memcpyable_impl {
                ...
                static constexpr bool value
                    = std::is_trivially_constructible<QualifiedTo, QualifiedFrom>::value
                      && std::is_trivially_copyable<to>::value
                      && (std::is_same<typename std::remove_cv<from>::type, to>::value
                          || is_memcpyable_integral<from, to>::value
                          || is_convertible_pointer<from, to>::value)
            ...
            template <typename To, typename... Args> struct is_uninitialized_memcpyable
                : std::false_type {};

            template <typename To, typename From> struct is_uninitialized_memcpyable<To, From>
                : is_uninitialized_memcpyable_impl<From, To> {};

            template <typename Iterator> struct is_small_vector_iterator : std::false_type {};

            template <typename... Ts> struct is_small_vector_iterator<small_vector_iterator<Ts...>>
                : std::true_type {};

            template <typename InputIt> struct is_contiguous_iterator
                : bool_constant<
                      std::is_same<InputIt, ptr>::value || std::is_same<InputIt, cptr>::value
                      ...
                      || (!std::is_same<value_ty, bool>
                          && (std::is_same<InputIt, typename std::vector<value_ty>::iterator>::value
                              || std::is_same<
                                  InputIt, typename std::vector<value_ty>::const_iterator>::value))
                      || std::is_same<InputIt,
                                      decltype(std::begin(
                                          std::declval<std::valarray<value_ty>&>()))>::value
                      || std::is_same<InputIt,
                                      decltype(std::begin(
                                          std::declval<const std::valarray<value_ty>&>()))>::value
#endif
                      > {
            };

            template <typename InputIt> struct is_memcpyable_iterator
                : bool_constant<is_memcpyable<decltype(*std::declval<InputIt>())>::value
                                && is_contiguous_iterator<InputIt>::value> {};

            // Unwrap `move_iterator`s.
            template <typename InputIt> struct is_memcpyable_iterator<std::move_iterator<InputIt>>
                : is_memcpyable_iterator<InputIt> {};

            template <typename InputIt, typename V = value_ty>
            struct is_uninitialized_memcpyable_iterator
                : bool_constant<
                      is_uninitialized_memcpyable<V, decltype(*std::declval<InputIt>())>::value
                      && is_contiguous_iterator<InputIt>::value> {};
            ...
            GCH_NORETURN
            static GCH_CPP20_CONSTEXPR void throw_range_length_error(void) {
#ifdef GCH_EXCEPTIONS
            ...
            static constexpr value_ty* to_address(value_ty* p) noexcept {
                static_assert(!std::is_function<value_ty>::value,
            ...
            template <typename Pointer, typename std::enable_if<!has_ptr_traits_to_address<
            ...
            GCH_NODISCARD static GCH_CONSTEVAL std::size_t numeric_max(void) noexcept {
            ...
            GCH_NODISCARD
            static GCH_CPP17_CONSTEXPR size_ty internal_range_length(cptr first,
                                                                     cptr last) noexcept {
            ...
            GCH_NODISCARD static GCH_CPP17_CONSTEXPR size_ty external_range_length_impl(
...
#ifndef NDEBUG
                if (numeric_max<size_ty>() < len) throw_range_length_error();
#endif
                return static_cast<size_ty>(len);
            }

            template <typename ForwardIt> GCH_NODISCARD static GCH_CPP17_CONSTEXPR size_ty
            ...
            template <typename ForwardIt,
                      ...
                      typename std::enable_if<(numeric_max<size_ty>() < numeric_max<ItDiffT>()),
                                              bool>::type
                      = true>
            GCH_NODISCARD static GCH_CPP17_CONSTEXPR size_ty external_range_length(ForwardIt first,
                                                                                   ForwardIt last) {
            ...
            template <typename ForwardIt,
                      ...
                      typename std::enable_if<!(numeric_max<size_ty>() < numeric_max<ItDiffT>()),
                                              bool>::type
                      = false>
            GCH_NODISCARD static GCH_CPP17_CONSTEXPR size_ty
            ...
            GCH_NODISCARD static GCH_CPP17_CONSTEXPR Iterator unchecked_next(Iterator pos,
                                                                             Integer n
                                                                             = 1) noexcept {
            ...
            template <typename Iterator,
                      ...
                      = typename std::iterator_traits<Iterator>::difference_type,
                      typename Integer = IteratorDiffT>
            GCH_NODISCARD static GCH_CPP17_CONSTEXPR Iterator unchecked_prev(Iterator pos,
                                                                             Integer n
                                                                             = 1) noexcept {
                unchecked_advance(pos, -static_cast<IteratorDiffT>(n));
                return pos;
            ...
            template <typename Iterator,
                      ...
                      = typename std::iterator_traits<Iterator>::difference_type,
                      typename Integer = IteratorDiffT>
            static GCH_CPP17_CONSTEXPR void unchecked_advance(Iterator& pos, Integer n) noexcept {
                std::advance(pos, static_cast<IteratorDiffT>(n));
            }

            GCH_NODISCARD GCH_CPP20_CONSTEXPR size_ty get_max_size(void) const noexcept {
            ...
            GCH_NODISCARD GCH_CPP20_CONSTEXPR ptr allocate(size_ty n) {
                return alloc_traits::allocate(allocator_ref(), static_cast<size_type>(n));
            }

            GCH_NODISCARD GCH_CPP20_CONSTEXPR ptr allocate_with_hint(size_ty n, cptr hint) {
                return alloc_traits::allocate(allocator_ref(), static_cast<size_type>(n), hint);
            }

            GCH_CPP20_CONSTEXPR
            void deallocate(ptr p, size_ty n) {
                alloc_traits::deallocate(allocator_ref(), to_address(p), static_cast<size_type>(n));
            }

            template <typename U, typename std::enable_if<is_uninitialized_memcpyable<
                                      value_ty, U>::value>::type* = nullptr>
            GCH_CPP20_CONSTEXPR void construct(ptr p, U&& val) noexcept {
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
                if (std::is_constant_evaluated()) {
            ...
            template <typename A = alloc_ty, typename V = value_ty, typename... Args,
                      typename std::enable_if<
                          (sizeof...(Args) != 1 || !is_uninitialized_memcpyable<V, Args...>::value)
                          && has_alloc_construct<A, V, Args...>::value>::type* = nullptr>
            GCH_CPP20_CONSTEXPR void construct(ptr p, Args&&... args) noexcept(noexcept(
                alloc_traits::construct(std::declval<alloc_ty&>(), std::declval<value_ty*>(),
            ...
            template <
                typename A = alloc_ty, typename V = value_ty, typename... Args,
            ...
            template <
                ...
                typename std::enable_if<is_trivially_destructible<V>::value
                                        && !must_use_alloc_destroy<A, V>::value>::type* = nullptr>
            GCH_CPP14_CONSTEXPR void destroy_range(ptr, ptr) const noexcept {}

            template <
                typename A = alloc_ty, typename V = value_ty,
                typename std::enable_if<!is_trivially_destructible<V>::value
                                        || must_use_alloc_destroy<A, V>::value>::type* = nullptr>
            GCH_CPP20_CONSTEXPR void destroy_range(ptr first, ptr last) noexcept {
            ...
            template <typename ForwardIt,
                      typename std::enable_if<
                          is_uninitialized_memcpyable_iterator<ForwardIt>::value, bool>::type
                      = true>
            GCH_CPP20_CONSTEXPR ptr uninitialized_copy(ForwardIt first, ForwardIt last,
                                                       ptr dest) noexcept {
            ...
            template <typename ForwardIt,
                      typename std::enable_if<
                          is_uninitialized_memcpyable_iterator<ForwardIt>::value, bool>::type
                      = true>
            GCH_CPP20_CONSTEXPR ptr uninitialized_copy(std::move_iterator<ForwardIt> first,
            ...
            template <typename InputIt,
                      typename std::enable_if<!is_uninitialized_memcpyable_iterator<InputIt>::value,
                                              bool>::type
                      = false>
            GCH_CPP20_CONSTEXPR ptr uninitialized_copy(InputIt first, InputIt last, ptr d_first) {
                return default_uninitialized_copy(first, last, d_first);
            }

            template <typename InputIt>
            GCH_CPP20_CONSTEXPR ptr default_uninitialized_copy(InputIt first, InputIt last,
                                                               ptr d_first) {
                ptr d_last = d_first;
                GCH_TRY {
            ...
            GCH_CPP20_CONSTEXPR ptr uninitialized_value_construct(ptr first, ptr last) {
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
                if (std::is_constant_evaluated())
            ...
            GCH_CPP20_CONSTEXPR
            ptr default_uninitialized_value_construct(ptr first, ptr last) {
                ptr curr = first;
                GCH_TRY {
                    for (; !(curr == last); ++curr) construct(curr);
                    return curr;
                }
                GCH_CATCH(...) {
            ...
            GCH_CPP20_CONSTEXPR
            ptr uninitialized_fill(ptr first, ptr last) {
                return uninitialized_value_construct(first, last);
            }

            GCH_CPP20_CONSTEXPR
            ptr uninitialized_fill(ptr first, ptr last, const value_ty& val) {
                ptr curr = first;
                GCH_TRY {
                    for (; !(curr == last); ++curr) construct(curr, val);
                    return curr;
                }
                GCH_CATCH(...) {
                    destroy_range(first, curr);
          ...
          private:
            ...
            static GCH_CPP20_CONSTEXPR void destroy_at(value_ty* p) noexcept {
                for (auto& e : *p) destroy_at(std::addressof(e));
            }
            ...
            static GCH_CPP20_CONSTEXPR auto construct_at(value_ty* p, Args&&... args) noexcept(
        ...
        template <typename Pointer, typename SizeT> class small_vector_data_base {
          public:
            using ptr = Pointer;
            using size_ty = SizeT;
            ...
            GCH_CPP20_CONSTEXPR void set_capacity(size_ty capacity) noexcept {
                m_capacity = capacity;
            }
            GCH_CPP20_CONSTEXPR void set_size(size_ty size) noexcept { m_size = size; }

            GCH_CPP20_CONSTEXPR
            void set(ptr data_ptr, size_ty capacity, size_ty size) {
                m_data_ptr = data_ptr;
                m_capacity = capacity;
                m_size = size;
            }
            ...
            GCH_CPP20_CONSTEXPR
            void swap_data_ptr(small_vector_data_base& other) noexcept {
                using std::swap;
                swap(m_data_ptr, other.m_data_ptr);
            }

            GCH_CPP20_CONSTEXPR
            void swap_capacity(small_vector_data_base& other) noexcept {
                using std::swap;
                swap(m_capacity, other.m_capacity);
            }

            GCH_CPP20_CONSTEXPR
            void swap_size(small_vector_data_base& other) noexcept {
                using std::swap;
                swap(m_size, other.m_size);
            }

            GCH_CPP20_CONSTEXPR
            void swap(small_vector_data_base& other) noexcept {
                using std::swap;
                swap(m_data_ptr, other.m_data_ptr);
                swap(m_capacity, other.m_capacity);
                swap(m_size, other.m_size);
        ...
        class small_vector_data : public small_vector_data_base<Pointer, SizeT> {
          public:
            ...
            GCH_CPP14_CONSTEXPR
            value_ty* storage(void) noexcept { return m_storage.get_inline_ptr(); }

            constexpr const value_ty* storage(void) const noexcept {
        ...
        class GCH_EMPTY_BASE small_vector_data<Pointer, SizeT, T, 0>
            : public small_vector_data_base<Pointer, SizeT>, private inline_storage<T, 0> {
          ...
          public:
            ...
            GCH_CPP14_CONSTEXPR
            value_ty* storage(void) noexcept { return base::get_inline_ptr(); }
        ...
        template <typename Allocator, unsigned InlineCapacity> class small_vector_base
          ...
          protected:
            ...
            static_assert(alloc_interface::template is_complete<value_ty>::value
                              || InlineCapacity == 0,
                          "`value_type` must be complete for instantiation of a non-zero number "
                          "of inline elements.");

            template <typename T> using is_complete =
                typename alloc_interface::template is_complete<T>;
            ...
            GCH_NODISCARD
            static GCH_CONSTEVAL size_ty get_inline_capacity(void) noexcept {
            ...
            struct is_emplace_constructible_impl<
                void_t<typename std::enable_if<is_complete<V>::value>::type,
                       decltype(std::declval<AI&>().construct(std::declval<V*>(),
                                                              std::declval<Args>()...))>,
                AI, V, Args...> : std::true_type {
                using nothrow = bool_constant<noexcept(
                    std::declval<AI&>().construct(std::declval<V*>(), std::declval<Args>()...))>;
            };

            template <typename... Args> struct is_emplace_constructible
                : is_emplace_constructible_impl<void, alloc_interface, value_ty, Args...> {};

            template <typename... Args> struct is_nothrow_emplace_constructible
                : is_emplace_constructible_impl<void, alloc_interface, value_ty, Args...>::nothrow {
            };

            template <typename V = value_ty> struct is_explicitly_move_insertable
                : is_emplace_constructible<V&&> {};

            template <typename V = value_ty> struct is_explicitly_nothrow_move_insertable
                : is_nothrow_emplace_constructible<V&&> {};

            template <typename V = value_ty> struct is_explicitly_copy_insertable
                : std::integral_constant<bool, is_emplace_constructible<V&>::value
                                                   && is_emplace_constructible<const V&>::value> {};

            template <typename V = value_ty> struct is_explicitly_nothrow_copy_insertable
                : std::integral_constant<bool,
                                         is_nothrow_emplace_constructible<V&>::value
                                             && is_nothrow_emplace_constructible<const V&>::value> {
            };

            template <typename AI, typename Enable = void> struct is_eraseable : std::false_type {};

            template <typename AI> struct is_eraseable<
                AI, void_t<decltype(std::declval<AI&>().destroy(std::declval<value_ty*>()))>>
                : std::true_type {};

            template <typename V> struct relocate_with_move
            ...
            template <typename A> struct allocations_are_movable
                : bool_constant<
            ...
            template <typename A> struct allocations_are_swappable
            ...
            template <typename... Args> using is_memcpyable =
                typename alloc_interface::template is_memcpyable<Args...>;

            template <typename... Args> using is_memcpyable_iterator =
                typename alloc_interface::template is_memcpyable_iterator<Args...>;

            GCH_NORETURN
            static GCH_CPP20_CONSTEXPR void throw_overflow_error(void) {
#ifdef GCH_EXCEPTIONS
                throw std::overflow_error("The requested conversion would overflow.");
#else
                std::fprintf(stderr,
                             "[gch::small_vector] The requested conversion would overflow.\n");
                std::abort();
#endif
            }

            GCH_NORETURN
            static GCH_CPP20_CONSTEXPR void throw_index_error(void) {
#ifdef GCH_EXCEPTIONS
            ...
            GCH_NORETURN
            static GCH_CPP20_CONSTEXPR void throw_increment_error(void) {
#ifdef GCH_EXCEPTIONS
                throw std::domain_error(
                    "The requested increment was outside of the allowed range.");
#else
                std::fprintf(stderr,
                             "[gch::small_vector] The requested increment was outside of the "
                             "allowed range.\n");
                std::abort();
#endif
            }
            ...
            GCH_NORETURN
            static GCH_CPP20_CONSTEXPR void throw_allocation_size_error(void) {
#ifdef GCH_EXCEPTIONS
            ...
            GCH_NODISCARD GCH_CPP14_CONSTEXPR ptr
          ...
          private:
            class stack_temporary {
              public:
                ...
                GCH_CPP20_CONSTEXPR explicit stack_temporary(alloc_interface& alloc_iface,
                                                             Args&&... args)
                ...
                GCH_CPP20_CONSTEXPR
                ~stack_temporary(void) { m_interface.destroy(get_pointer()); }

                GCH_NODISCARD GCH_CPP20_CONSTEXPR const value_ty& get(void) const noexcept {
                    return *get_pointer();
                }

                GCH_NODISCARD GCH_CPP20_CONSTEXPR value_ty&& release(void) noexcept {
                    return std::move(*get_pointer());
                }

              private:
                GCH_NODISCARD GCH_CPP20_CONSTEXPR cptr get_pointer(void) const noexcept {
                    return static_cast<cptr>(static_cast<const void*>(std::addressof(m_data)));
                }

                GCH_NODISCARD GCH_CPP20_CONSTEXPR ptr get_pointer(void) noexcept {
            ...
            class heap_temporary {
              public:
                ...
                template <typename... Args>
                GCH_CPP20_CONSTEXPR explicit heap_temporary(alloc_interface& alloc_iface,
                                                            Args&&... args)
                    : m_interface(alloc_iface), m_data_ptr(alloc_iface.allocate(sizeof(value_ty))) {
                    GCH_TRY { m_interface.construct(m_data_ptr, std::forward<Args>(args)...); }
                    GCH_CATCH(...) {
            ...
            void wipe(void) {
                ...
                if (has_allocation()) deallocate(data_ptr(), get_capacity());
            }

            GCH_CPP20_CONSTEXPR
            void set_data_ptr(ptr data_ptr) noexcept { m_data.set_data_ptr(data_ptr); }

            GCH_CPP20_CONSTEXPR
            void set_capacity(size_ty capacity) noexcept {
                m_data.set_capacity(static_cast<size_type>(capacity));
            }

            GCH_CPP20_CONSTEXPR
            void set_size(size_ty size) noexcept { m_data.set_size(static_cast<size_type>(size)); }

            GCH_CPP20_CONSTEXPR
            void set_data(ptr data_ptr, size_ty capacity, size_ty size) noexcept {
                m_data.set(data_ptr, static_cast<size_type>(capacity),
                           static_cast<size_type>(size));
            }

            GCH_CPP20_CONSTEXPR
            void swap_data_ptr(small_vector_base& other) noexcept {
                m_data.swap_data_ptr(other.m_data);
            }

            GCH_CPP20_CONSTEXPR
            void swap_capacity(small_vector_base& other) noexcept {
                m_data.swap_capacity(other.m_data);
            }

            GCH_CPP20_CONSTEXPR
            void swap_size(small_vector_base& other) noexcept { m_data.swap_size(other.m_data); }

            GCH_CPP20_CONSTEXPR
            void swap_allocation(small_vector_base& other) noexcept { m_data.swap(other.m_data); }

            GCH_CPP20_CONSTEXPR
            void reset_data(ptr data_ptr, size_ty capacity, size_ty size) {
                wipe();
                m_data.set(data_ptr, static_cast<size_type>(capacity),
                           static_cast<size_type>(size));
            }
            ...
            GCH_CPP20_CONSTEXPR
            void increase_size(size_ty n) noexcept { m_data.set_size(get_size() + n); }

            GCH_CPP20_CONSTEXPR
            void decrease_size(size_ty n) noexcept { m_data.set_size(get_size() - n); }

            GCH_CPP20_CONSTEXPR
            ptr unchecked_allocate(size_ty n) {
            ...
            GCH_CPP20_CONSTEXPR
            ptr unchecked_allocate(size_ty n, cptr hint) {
                assert(InlineCapacity < n
                       && "Allocated capacity should be greater than InlineCapacity.");
            ...
            GCH_CPP20_CONSTEXPR
            ptr checked_allocate(size_ty n) {
                if (get_max_size() < n) throw_allocation_size_error();
                return unchecked_allocate(n);
            }

          protected:
            GCH_NODISCARD GCH_CPP14_CONSTEXPR size_ty unchecked_calculate_new_capacity(
                ...
                if (get_max_size() - current_capacity <= current_capacity) return get_max_size();

                // Note: This growth factor might be theoretically superior, but in testing it falls
                // flat: size_ty new_capacity = current_capacity + (current_capacity / 2);

                const size_ty new_capacity = 2 * current_capacity;
                if (new_capacity < minimum_required_capacity) return minimum_required_capacity;
                return new_capacity;
            }

            GCH_NODISCARD GCH_CPP14_CONSTEXPR size_ty
            checked_calculate_new_capacity(const size_ty minimum_required_capacity) const {
                if (get_max_size() < minimum_required_capacity) throw_allocation_size_error();
                return unchecked_calculate_new_capacity(minimum_required_capacity);
            }

            template <unsigned I> GCH_CPP20_CONSTEXPR small_vector_base& copy_assign_default(
...
#endif
                                              >::type* = nullptr>
            GCH_CPP20_CONSTEXPR small_vector_base& copy_assign(
                const small_vector_base<Allocator, I>& other) {
                if (other.allocator_ref() == allocator_ref()) return copy_assign_default(other);
            ...
            template <unsigned I> GCH_CPP20_CONSTEXPR void move_allocation_pointer(
            ...
            GCH_CPP20_CONSTEXPR small_vector_base&
            move_assign_default(small_vector_base<Allocator, LessEqualI>&& other) noexcept(
                std::is_nothrow_move_assignable<value_ty>::value&&
                ...
                // the inline capacity of `*this` because allocations can never have a smaller
                // capacity than the inline capacity.
                if (InlineCapacity < other.get_capacity())
                    ...
                    // Note: We do not need to deallocate any allocations in `other` because the
                    // value of
                    //       an object meeting the Allocator named requirements does not change
                    //       value after a move.
                }
            ...
            template <unsigned I>
            GCH_CPP20_CONSTEXPR small_vector_base& move_assign_unequal_no_propagate(
            ...
            GCH_CPP20_CONSTEXPR small_vector_base&
            move_assign(small_vector_base<Allocator, I>&& other) noexcept(noexcept(
            ...
            template <unsigned I, typename A = alloc_ty,
                      typename std::enable_if<!allocations_are_movable<A>::value>::type* = nullptr>
            GCH_CPP20_CONSTEXPR small_vector_base& move_assign(
                small_vector_base<Allocator, I>&& other) {
            ...
            GCH_CPP20_CONSTEXPR void move_initialize(small_vector_base&& other) noexcept {
            ...
            template <unsigned GreaterI,
                      typename std::enable_if<(InlineCapacity < GreaterI)>::type* = nullptr>
            GCH_CPP20_CONSTEXPR void move_initialize(
                small_vector_base<Allocator, GreaterI>&& other) {
                if (other.has_allocation()) {
                    set_data(other.data_ptr(), other.get_capacity(), other.get_size());
                    other.set_default();
          ...
          public:
            ...
            GCH_CPP20_CONSTEXPR
            small_vector_base(void) noexcept { set_default(); }

            static constexpr struct bypass_tag {
            } bypass{};

            template <unsigned I, typename... MaybeAlloc>
            GCH_CPP20_CONSTEXPR small_vector_base(bypass_tag,
                                                  const small_vector_base<Allocator, I>& other,
                ...
                if (InlineCapacity < other.get_size()) {
                    ...
                    GCH_TRY { uninitialized_copy(other.begin_ptr(), other.end_ptr(), data_ptr()); }
                    GCH_CATCH(...) {
                        deallocate(data_ptr(), get_capacity());
                        GCH_THROW;
                    }
            ...
            template <unsigned I> GCH_CPP20_CONSTEXPR
            small_vector_base(bypass_tag, small_vector_base<Allocator, I>&& other) noexcept(
                std::is_nothrow_move_constructible<value_ty>::value
            ...
            GCH_CPP20_CONSTEXPR small_vector_base(
                ...
                const alloc_ty&) noexcept(noexcept(small_vector_base(bypass, std::move(other))))
            ...
            GCH_CPP20_CONSTEXPR small_vector_base(bypass_tag,
                ...
                if (InlineCapacity < other.get_size()) {
                    ...
                    GCH_TRY { uninitialized_move(other.begin_ptr(), other.end_ptr(), data_ptr()); }
                    GCH_CATCH(...) {
                        deallocate(data_ptr(), get_capacity());
                        GCH_THROW;
                    }
            ...
            GCH_CPP20_CONSTEXPR explicit small_vector_base(const alloc_ty& alloc) noexcept
                : alloc_interface(alloc) {
                set_default();
            }

            GCH_CPP20_CONSTEXPR
            small_vector_base(size_ty count, const alloc_ty& alloc) : alloc_interface(alloc) {
            ...
            GCH_CPP20_CONSTEXPR small_vector_base(size_ty count, Generator& g,
                                                  const alloc_ty& alloc)
                : alloc_interface(alloc) {
                if (InlineCapacity < count) {
                    set_data_ptr(checked_allocate(count));
                ...
                GCH_TRY {
                    for (; !(curr == new_end); ++curr) construct(curr, g());
                }
                GCH_CATCH(...) {
            ...
            template <typename InputIt>
#endif
            GCH_CPP20_CONSTEXPR small_vector_base(InputIt first, InputIt last,
            ...
            template <typename ForwardIt>
#endif
            GCH_CPP20_CONSTEXPR small_vector_base(ForwardIt first, ForwardIt last,
                ...
                if (InlineCapacity < count) {
                    ...
                    set_capacity(count);
                    GCH_TRY { uninitialized_copy(first, last, begin_ptr()); }
                    GCH_CATCH(...) {
                        deallocate(data_ptr(), get_capacity());
                        GCH_THROW;
                    }
          ...
          protected:
            GCH_CPP20_CONSTEXPR
            void set_to_inline_storage(void) {
                set_capacity(InlineCapacity);
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
                if (std::is_constant_evaluated())
            ...
            GCH_CPP20_CONSTEXPR
            void assign_with_copies(size_ty count, const value_ty& val) {
            ...
            template <typename InputIt,
                      typename std::enable_if<std::is_assignable<
                          value_ty&, decltype(*std::declval<InputIt>())>::value>::type* = nullptr>
            GCH_CPP20_CONSTEXPR void assign_with_range(InputIt first, InputIt last,
            ...
            template <typename ForwardIt,
                      typename std::enable_if<std::is_assignable<
                          value_ty&, decltype(*std::declval<ForwardIt>())>::value>::type* = nullptr>
            GCH_CPP20_CONSTEXPR void assign_with_range(ForwardIt first, ForwardIt last,
                ...
                if (get_capacity() < count) {
                    ...
                    GCH_TRY { uninitialized_copy(first, last, new_begin); }
                    GCH_CATCH(...) {
                        deallocate(new_begin, new_capacity);
                        GCH_THROW;
                    }
            ...
            template <typename Policy = void, typename V = value_ty,
                      typename std::enable_if<
                          is_explicitly_move_insertable<V>::value
                              && (!std::is_same<Policy, strong_exception_policy>::value
                                  || relocate_with_move<V>::value),
                          bool>::type
                      = true>
            GCH_CPP20_CONSTEXPR ptr uninitialized_move(ptr first, ptr last, ptr d_first) noexcept(
                std::is_nothrow_move_constructible<value_ty>::value) {
            ...
            template <
                typename Policy = void, typename V = value_ty,
                ...
                = false>
            GCH_CPP20_CONSTEXPR ptr uninitialized_move(ptr first, ptr last, ptr d_first) noexcept(
                alloc_interface::template is_uninitialized_memcpyable_iterator<ptr>::value) {
                return uninitialized_copy(first, last, d_first);
            }

            GCH_CPP20_CONSTEXPR
            ptr shift_into_uninitialized(ptr pos, size_ty n_shift) {
            ...
            template <typename... Args> GCH_CPP20_CONSTEXPR ptr append_element(Args&&... args) {
            ...
            GCH_CPP20_CONSTEXPR
            ptr append_copies(size_ty count, const value_ty& val) {
                if (num_uninitialized() < count) {
                    // Reallocate.
                    if (get_max_size() - get_size() < count) throw_allocation_size_error();
            ...
            template <typename MovePolicy, typename InputIt,
                      typename std::enable_if<
                          std::is_same<MovePolicy, strong_exception_policy>::value, bool>::type
                      = true>
            GCH_CPP20_CONSTEXPR ptr append_range(InputIt first, InputIt last,
                                                 std::input_iterator_tag) {
                // Append with a strong exception guarantee.
                size_ty original_size = get_size();
                for (; !(first == last); ++first) {
                    GCH_TRY { append_element(*first); }
                    GCH_CATCH(...) {
            ...
            GCH_CPP20_CONSTEXPR ptr append_range(InputIt first, InputIt last,
                                                 std::input_iterator_tag) {
                size_ty original_size = get_size();
                for (; !(first == last); ++first) append_element(*first);
                return unchecked_next(begin_ptr(), original_size);
            }

            template <typename MovePolicy = void, typename ForwardIt>
            GCH_CPP20_CONSTEXPR ptr append_range(ForwardIt first, ForwardIt last,
                                                 std::forward_iterator_tag) {
                const size_ty num_insert = external_range_length(first, last);

                if (num_uninitialized() < num_insert) {
                    // Reallocate.
                    if (get_max_size() - get_size() < num_insert) throw_allocation_size_error();
            ...
            template <typename... Args>
            GCH_CPP20_CONSTEXPR ptr emplace_at(ptr pos, Args&&... args) {
            ...
            GCH_CPP20_CONSTEXPR
            ptr insert_copies(ptr pos, size_ty count, const value_ty& val) {
                if (0 == count) return pos;

                if (end_ptr() == pos) {
                    if (1 == count) return append_element(val);
                ...
                if (num_uninitialized() < count) {
                    ...
                    if (tail_size < count) {
                        ...
                        GCH_TRY { std::fill(pos, inserted_end, tmp.get()); }
                        GCH_CATCH(...) {
            ...
            template <typename ForwardIt>
            GCH_CPP20_CONSTEXPR ptr insert_range_helper(ptr pos, ForwardIt first, ForwardIt last) {
                ...
                if (num_uninitialized() < num_insert) {
                    ...
                    if (tail_size < num_insert) {
                        ...
                        // Attempt to copy over the elements.
                        // If we fail we'll attempt a full roll-back.
                        GCH_TRY { copy_range(first, last, pos); }
                        GCH_CATCH(...) {
            ...
            template <typename InputIt>
            GCH_CPP20_CONSTEXPR ptr insert_range(ptr pos, InputIt first, InputIt last,
                                                 std::input_iterator_tag) {
                assert(!(first == last) && "The range should not be empty.");

                // Ensure we use this specific overload to give a strong exception guarantee for 1
                // element.
                if (end_ptr() == pos) return append_range(first, last, std::input_iterator_tag{});
            ...
            template <typename ForwardIt>
            GCH_CPP20_CONSTEXPR ptr insert_range(ptr pos, ForwardIt first, ForwardIt last,
                                                 std::forward_iterator_tag) {
                if (!(end_ptr() == pos)) return insert_range_helper(pos, first, last);

                if (unchecked_next(first) == last) return append_element(*first);
            ...
            template <typename... Args>
            GCH_CPP20_CONSTEXPR ptr emplace_into_current_end(Args&&... args) {
            ...
            GCH_CPP20_CONSTEXPR ptr emplace_into_current(ptr pos, value_ty&& val) {
                if (pos == end_ptr()) return emplace_into_current_end(std::move(val));

                // In the special case of value_ty&& we don't make a copy because behavior is
                // unspecified when it is an internal element. Hence, we'll take the opportunity to
                // optimize and assume that it isn't an internal element.
            ...
            GCH_CPP20_CONSTEXPR ptr emplace_into_current(ptr pos, Args&&... args) {
                if (pos == end_ptr()) return emplace_into_current_end(std::forward<Args>(args)...);
            ...
            template <typename... Args>
            GCH_CPP20_CONSTEXPR ptr emplace_into_reallocation_end(Args&&... args) {
                // Appending; strong exception guarantee.
                if (get_max_size() == get_size()) throw_allocation_size_error();
            ...
            GCH_CPP20_CONSTEXPR
            ptr shrink_to_size(void) {
                if (!has_allocation() || get_size() == get_capacity()) return begin_ptr();
            ...
            GCH_CPP20_CONSTEXPR void resize_with(size_ty new_size, const ValueT&... val) {
                // ValueT... should either be value_ty or empty.

                if (new_size == 0) erase_all();

                if (get_capacity() < new_size) {
                    // Reallocate.

                    if (get_max_size() < new_size) throw_allocation_size_error();
            ...
            GCH_CPP20_CONSTEXPR
            void request_capacity(size_ty request) {
                if (request <= get_capacity()) return;
            ...
            GCH_CPP20_CONSTEXPR
            ptr erase_range(ptr first, ptr last) {
                if (!(first == last)) erase_to_end(move_left(last, end_ptr(), first));
                return first;
            }
            ...
            GCH_CPP20_CONSTEXPR
            void swap_unequal_no_propagate(small_vector_base& other) {
                assert(get_capacity() <= other.get_capacity());
            ...
            GCH_CPP20_CONSTEXPR void swap(small_vector_base& other) {
                if (get_capacity() < other.get_capacity()) {
                    if (other.allocator_ref() == allocator_ref())
                        swap_default(other);
                    else
            ...
            template <typename InputIt>
            static GCH_CPP20_CONSTEXPR InputIt unmove_iterator(InputIt it) {
                return it;
            }

            template <typename InputIt>
            static GCH_CPP20_CONSTEXPR auto unmove_iterator(std::move_iterator<InputIt> it)
                -> decltype(unmove_iterator(it.base())) {
                return unmove_iterator(it.base());
            }

            template <typename InputIt>
            static GCH_CPP20_CONSTEXPR auto unmove_iterator(std::reverse_iterator<InputIt> it)
            ...
            template <typename InputIt>
            GCH_CPP20_CONSTEXPR ptr copy_range(InputIt first, InputIt last, ptr dest) {
            ...
            GCH_CPP20_CONSTEXPR InputIt copy_n_return_in(InputIt first, size_ty count,
                                                         ptr dest) noexcept {
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
                if (std::is_constant_evaluated()) {
            ...
            template <
                typename InputIt,
                typename std::enable_if<is_memcpyable_iterator<InputIt>::value>::type* = nullptr>
            GCH_CPP20_CONSTEXPR std::move_iterator<InputIt> copy_n_return_in(
            ...
            template <typename RandomIt,
                      typename std::enable_if<
                          ...
                          && std::is_base_of<std::random_access_iterator_tag,
                                             typename std::iterator_traits<RandomIt>::
                                                 iterator_category>::value>::type* = nullptr>
            GCH_CPP20_CONSTEXPR RandomIt copy_n_return_in(RandomIt first, size_ty count, ptr dest) {
#if defined(GCH_LIB_IS_CONSTANT_EVALUATED) && defined(__GLIBCXX__)
                if (std::is_constant_evaluated()
                      ...
                      typename std::enable_if<
                          ...
                          && !std::is_base_of<std::random_access_iterator_tag,
                                              typename std::iterator_traits<InputIt>::
                                                  iterator_category>::value>::type* = nullptr>
            GCH_CPP20_CONSTEXPR InputIt copy_n_return_in(InputIt first, size_ty count, ptr dest) {
            ...
            GCH_CPP20_CONSTEXPR ptr move_left(ptr first, ptr last, ptr d_first) {
                // Shift initialized elements to the left.

#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
                if (std::is_constant_evaluated()) return std::move(first, last, d_first);
#endif
            ...
            GCH_CPP20_CONSTEXPR ptr move_right(ptr first, ptr last, ptr d_last) {
                // Move initialized elements to the right.

#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
                if (std::is_constant_evaluated()) return std::move_backward(first, last, d_last);
#endif
          ...
          public:
            ...
            GCH_NODISCARD GCH_CPP14_CONSTEXPR ptr data_ptr(void) noexcept {
                return m_data.data_ptr();
            }

            GCH_NODISCARD constexpr cptr data_ptr(void) const noexcept { return m_data.data_ptr(); }

            GCH_NODISCARD constexpr size_ty get_capacity(void) const noexcept {
                return m_data.capacity();
            }

            GCH_NODISCARD constexpr size_ty get_size(void) const noexcept { return m_data.size(); }

            GCH_NODISCARD constexpr size_ty num_uninitialized(void) const noexcept {
                return get_capacity() - get_size();
            }

            GCH_NODISCARD GCH_CPP14_CONSTEXPR ptr begin_ptr(void) noexcept { return data_ptr(); }

            GCH_NODISCARD
            constexpr cptr begin_ptr(void) const noexcept { return data_ptr(); }

            GCH_NODISCARD GCH_CPP14_CONSTEXPR ptr end_ptr(void) noexcept {
                return unchecked_next(begin_ptr(), get_size());
            }

            GCH_NODISCARD constexpr cptr end_ptr(void) const noexcept {
                return unchecked_next(begin_ptr(), get_size());
            }

            GCH_NODISCARD GCH_CPP14_CONSTEXPR ptr allocation_end_ptr(void) noexcept {
                return unchecked_next(begin_ptr(), get_capacity());
            }

            GCH_NODISCARD constexpr cptr allocation_end_ptr(void) const noexcept {
                return unchecked_next(begin_ptr(), get_capacity());
            }

            GCH_NODISCARD constexpr alloc_ty copy_allocator(void) const noexcept {
                return alloc_ty(allocator_ref());
            }

            GCH_NODISCARD GCH_CPP14_CONSTEXPR ptr storage_ptr(void) noexcept {
                return m_data.storage();
            }

            GCH_NODISCARD constexpr cptr storage_ptr(void) const noexcept {
                return m_data.storage();
            }

            GCH_NODISCARD constexpr bool has_allocation(void) const noexcept {
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
            ...
            GCH_NODISCARD constexpr bool is_inlinable(void) const noexcept {
    ...
    template <typename T, unsigned InlineCapacity, typename Allocator>
        ...
        requires concepts::small_vector::AllocatorFor<Allocator, T>
#endif
    class small_vector : private detail::small_vector_base<Allocator, InlineCapacity> {
      ...
      public:
        ...
        static_assert(
            InlineCapacity <= (std::numeric_limits<size_type>::max)(),
      ...
      private:
        static constexpr auto Destructible = concepts::small_vector::Destructible<value_type>;

        static constexpr auto MoveAssignable = concepts::small_vector::MoveAssignable<value_type>;

        static constexpr auto CopyAssignable = concepts::small_vector::CopyAssignable<value_type>;
        ...
        static constexpr auto Swappable = concepts::small_vector::Swappable<value_type>;

        static constexpr auto DefaultInsertable
            = concepts::small_vector::DefaultInsertable<value_type, small_vector, allocator_type>;

        static constexpr auto MoveInsertable
            = concepts::small_vector::MoveInsertable<value_type, small_vector, allocator_type>;

        static constexpr auto CopyInsertable
            = concepts::small_vector::CopyInsertable<value_type, small_vector, allocator_type>;

        static constexpr auto Erasable
            = concepts::small_vector::Erasable<value_type, small_vector, allocator_type>;

        template <typename... Args> struct EmplaceConstructible {
      ...
      public:
        ...
        GCH_CPP20_CONSTEXPR
        small_vector(void) noexcept(noexcept(allocator_type()))
#ifdef GCH_LIB_CONCEPTS
        ...
        template <typename Generator, typename std::enable_if<!std::is_convertible<
                                          Generator, const_reference>::value>::type* = nullptr>
#endif
        GCH_CPP20_CONSTEXPR small_vector(size_type count, Generator g,
                                         const allocator_type& alloc = allocator_type())
            : base(count, g, alloc) {
        }
        ...
        template <unsigned I>
            ...
            requires CopyInsertable
#endif
        GCH_CPP20_CONSTEXPR explicit small_vector(const small_vector<T, I, Allocator>& other)
            : base(base::bypass, other) {
        }
        ...
        template <unsigned I>
            ...
            requires MoveInsertable
#endif
        GCH_CPP20_CONSTEXPR explicit small_vector(small_vector<T, I, Allocator>&& other) noexcept(
        ...
        template <unsigned I>
            ...
            requires CopyInsertable
#endif
        GCH_CPP20_CONSTEXPR small_vector(const small_vector<T, I, Allocator>& other,
        ...
        template <unsigned I>
            ...
            requires MoveInsertable
#endif
        GCH_CPP20_CONSTEXPR small_vector(small_vector<T, I, Allocator>&& other,
                                         const allocator_type& alloc)
            : base(base::bypass, std::move(other), alloc) {
        }
        ...
        small_vector& operator=(small_vector&& other) noexcept(
            ...
            // false implies MoveInsertable && MoveAssignable, but since we have inline storage we
            // must always require moves [tab:container.alloc.req].
        ...
        GCH_CPP20_CONSTEXPR
        small_vector& operator=(std::initializer_list<value_type> ilist)
#ifdef GCH_LIB_CONCEPTS
            requires CopyInsertable && CopyAssignable
#endif
        {
        ...
        GCH_CPP20_CONSTEXPR
        void assign(const small_vector& other)
#ifdef GCH_LIB_CONCEPTS
            requires CopyInsertable && CopyAssignable
#endif
        {
            if (&other != this) base::copy_assign(other);
        }
        ...
        template <unsigned I>
            ...
            requires CopyInsertable && CopyAssignable
#endif
        GCH_CPP20_CONSTEXPR void assign(const small_vector<T, I, Allocator>& other) {
            base::copy_assign(other);
        }

        GCH_CPP20_CONSTEXPR
        void assign(small_vector&& other) noexcept(
            (std::is_same<std::allocator<value_type>, Allocator>::value
             || std::allocator_traits<Allocator>::propagate_on_container_move_assignment::value
#ifdef GCH_LIB_IS_ALWAYS_EQUAL
             || std::allocator_traits<Allocator>::is_always_equal::value
...
#endif
        {
            if (&other != this) base::move_assign(std::move(other));
        }
        ...
        template <unsigned I>
            ...
            requires MoveInsertable && MoveAssignable
#endif
        GCH_CPP20_CONSTEXPR void assign(small_vector<T, I, Allocator>&& other) noexcept(
            I <= InlineCapacity
            && (std::is_same<std::allocator<value_type>, Allocator>::value
                || std::allocator_traits<Allocator>::propagate_on_container_move_assignment::value
#ifdef GCH_LIB_IS_ALWAYS_EQUAL
...
#    endif
                          ...
                          && InlineCapacity == 0)>::type* = nullptr>
#endif
        GCH_CPP20_CONSTEXPR void swap(small_vector& other) noexcept(
            (std::is_same<std::allocator<value_type>, Allocator>::value
             || std::allocator_traits<Allocator>::propagate_on_container_swap::value
#ifdef GCH_LIB_IS_ALWAYS_EQUAL
             || std::allocator_traits<Allocator>::is_always_equal::value
...
#    endif
        ...
        GCH_CPP14_CONSTEXPR
        iterator begin(void) noexcept { return iterator{base::begin_ptr()}; }
        ...
        GCH_CPP14_CONSTEXPR
        iterator end(void) noexcept { return iterator{base::end_ptr()}; }
        ...
        GCH_CPP14_CONSTEXPR
        reference operator[](size_type pos) { return begin()[static_cast<difference_type>(pos)]; }
        ...
        constexpr const_reference front(void) const { return (*this)[0]; }

        GCH_CPP14_CONSTEXPR
        reference back(void) { return (*this)[size() - 1]; }

        constexpr const_reference back(void) const { return (*this)[size() - 1]; }

        GCH_CPP14_CONSTEXPR
        pointer data(void) noexcept { return base::begin_ptr(); }

        constexpr const_pointer data(void) const noexcept { return base::begin_ptr(); }

        constexpr size_type size(void) const noexcept {
            return static_cast<size_type>(base::get_size());
        }

        GCH_NODISCARD constexpr bool empty(void) const noexcept { return size() == 0; }
        ...
        GCH_CPP20_CONSTEXPR iterator insert(const_iterator pos, InputIt first, InputIt last) {
            if (first == last) return iterator(base::ptr_cast(pos));
        ...
        GCH_NODISCARD constexpr bool inlined(void) const noexcept {
            return !base::has_allocation();
        }

        GCH_NODISCARD constexpr bool inlinable(void) const noexcept { return base::is_inlinable(); }

        GCH_NODISCARD
        static GCH_CONSTEVAL size_type inline_capacity(void) noexcept {
        ...
        template <
            ...
            typename std::enable_if<std::is_base_of<
                ...
                typename std::iterator_traits<InputIt>::iterator_category>::value>::type* = nullptr>
#endif
        GCH_CPP20_CONSTEXPR small_vector& append(InputIt first, InputIt last) {
        ...
        GCH_CPP20_CONSTEXPR
        small_vector& append(std::initializer_list<value_type> ilist)
#ifdef GCH_LIB_CONCEPTS
            requires EmplaceConstructible<const_reference>::value && MoveInsertable
#endif
        {
        ...
        template <unsigned I>
        GCH_CPP20_CONSTEXPR small_vector& append(const small_vector<T, I, Allocator>& other)
#ifdef GCH_LIB_CONCEPTS
            requires CopyInsertable
#endif
        {
        ...
        template <unsigned I>
        GCH_CPP20_CONSTEXPR small_vector& append(small_vector<T, I, Allocator>&& other)
#ifdef GCH_LIB_CONCEPTS
            requires MoveInsertable
#endif
        {
    ...
    template <typename T, unsigned InlineCapacityLHS, unsigned InlineCapacityRHS,
              typename Allocator>
    inline GCH_CPP20_CONSTEXPR bool operator==(
    ...
    constexpr auto operator<=>(const small_vector<T, InlineCapacityLHS, Allocator>& lhs,
                               const small_vector<T, InlineCapacityRHS, Allocator>& rhs) {
        return std::lexicographical_compare_three_way(lhs.begin(), lhs.end(), rhs.begin(),
                                                      rhs.end(), std::compare_three_way{});
    }

    template <typename T, unsigned InlineCapacity, typename Allocator>
        requires std::three_way_comparable<T>
    constexpr auto operator<=>(const small_vector<T, InlineCapacity, Allocator>& lhs,
                               const small_vector<T, InlineCapacity, Allocator>& rhs) {
        return std::lexicographical_compare_three_way(lhs.begin(), lhs.end(), rhs.begin(),
                                                      rhs.end(), std::compare_three_way{});
    }
...
#    endif
...
#endif
              >
    inline GCH_CPP20_CONSTEXPR void swap(
        small_vector<T, InlineCapacity, Allocator>& lhs,
        small_vector<T, InlineCapacity, Allocator>& rhs) noexcept(noexcept(lhs.swap(rhs)))
#ifdef GCH_LIB_CONCEPTS
    ...
    inline GCH_CPP20_CONSTEXPR typename small_vector<T, InlineCapacity, Allocator>::size_type erase(
        small_vector<T, InlineCapacity, Allocator>& v, const U& value) {
    ...
    inline GCH_CPP20_CONSTEXPR typename small_vector<T, InlineCapacity, Allocator>::size_type
    ...
    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::const_iterator begin(
        const small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return v.begin();
    }

    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::const_iterator cbegin(
        const small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return begin(v);
    }
    ...
    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::const_iterator end(
        const small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return v.end();
    }

    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::const_iterator cend(
        const small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return end(v);
    }

    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::reverse_iterator rbegin(
        small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return v.rbegin();
    }

    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::const_reverse_iterator rbegin(
        const small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return v.rbegin();
    }

    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::const_reverse_iterator crbegin(
        const small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return rbegin(v);
    }

    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::reverse_iterator rend(
        small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return v.rend();
    }

    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::const_reverse_iterator rend(
        const small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return v.rend();
    }

    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::const_reverse_iterator crend(
        const small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return rend(v);
    }
    ...
    template <typename T, unsigned InlineCapacity, typename Allocator> constexpr
        typename std::common_type<std::ptrdiff_t,
                                  typename std::make_signed<typename small_vector<
                                      T, InlineCapacity, Allocator>::size_type>::type>::type
    ...
    template <typename T, unsigned InlineCapacity, typename Allocator>
    GCH_NODISCARD constexpr bool empty(
        const small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return v.empty();
    }
    ...
    template <typename T, unsigned InlineCapacity, typename Allocator>
    constexpr typename small_vector<T, InlineCapacity, Allocator>::const_pointer data(
        const small_vector<T, InlineCapacity, Allocator>& v) noexcept {
        return v.data();
    }
